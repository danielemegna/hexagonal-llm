...
class TestAIFoodFactsInterpreter(TestCase):
    interpreter = AIFoodFactsInterpreter("Qwen3.6-35B-A3B-4bit")

    def test_transform(self) -> None:
        food_facts = self.interpreter.transform(some_open_food_facts())
        assert {
            'cioccolato al latte',
            'nocciole',
            'zucchero',
            ...
            'sale',
            'vanillina'
        }.issubset(food_facts.ingredients)

def some_open_food_facts() -> OpenFoodFacts:
